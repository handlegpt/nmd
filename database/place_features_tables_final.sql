-- Final Place Features Database Setup
-- This version handles existing policies and avoids conflicts

-- Step 1: Create all tables (we know this works)
...
-- Step 2: Create indexes (we know the safe version works)
...
-- Step 3: Create function and triggers
...
-- Step 5: Create storage policies (with proper conflict handling)
...
DROP POLICY IF EXISTS "Users can update their own place photos" ON storage.objects;
...
DROP POLICY IF EXISTS "Users can delete their own place photos" ON storage.objects;
...
-- Final status
SELECT 'All place features setup completed successfully' as final_status;
